...
test-filter:
	...
	poetry run python -m unittest discover -p test_analysis.py -k TestDfAnalysis.test_get_df_set_analysis_result_for_several_df_analysis
	#poetry run python -m unittest discover -p test_with_local_s3_server.py -k TestWithLocalS3Server.test_run_test_s3_data
	#poetry run python -m unittest discover -p test_main.py -k TestFunction_runNoLocalS3Server.test_run_manages_analysis_config_error_and_generates_expected_error_messages
